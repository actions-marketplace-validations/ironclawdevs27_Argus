...
- `test-harness/validate.js` — 149-block correctness harness (846 hard assertions)
...
| **Test blocks** | 149 |
| **Hard assertions** | 846 |
| **Soft assertions** | ~23 executed/run (Lighthouse / perf-trace / heap — headless-unavailable; promoted to counted hard assertions in the weekly headful lane via `ARGUS_HARNESS_STRICT_SOFT`, Phase 4.3 — `harness-headful.yml`) |
| **Unit tests** | 94 (9 Vitest files, no Chrome — `npm run test:unit`) |
...
| **Harness gate** | **846/846** (no permanent failures — exits 0) |
| **Coverage gate** | merged unit (vitest-v8) + harness (c8) via `scripts/coverage-gate.mjs`; `--min-lines 60` (observed 75.0% lines) + `--allow-uncovered` zero-coverage-module guard; CI step in `harness-ci.yml` |
