...
- Harness statistics (149 blocks, 845 hard assertions, 67 detection categories)
...
test/
  unit/                       — 8 Vitest unit test files (72 tests); run with npm run test:unit
scripts/
...
npm run test:unit          # 72 Vitest unit tests — no Chrome required
npm run test:harness       # Expected: 845/845 (no permanent failures); INFO logs suppressed via .env.harness
