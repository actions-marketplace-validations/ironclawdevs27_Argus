...
├── test-harness/
│   ├── validate.js                   — 146-block correctness harness (757/757 gate)
│   ├── contracts/                    — golden Zod response schemas for the 9 MCP tools (block [147])
...
**None** — the harness passes `757/757` and `KNOWN_PERMANENT` in `validate.js` is empty (since v9.7.2). All three historical "permanent failures" were Argus bugs:
